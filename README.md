...
This is a proof of concept on how to send signed commands to Tesla vehicles and using StreamSession by making a WebSocket endpoint available.
...
```
# export TESLA_HTTP_JWT_TOKEN=$OWNER_AUTH_TOKEN
# go run ./cmd/tesla-http-proxy -tls-key key.pem -cert cert.pem -port 4443 -key-file private-key.pem -mode owner -verbose
```

A websocket server can be found at `wss://localhost:4443/api/1/vehicles/YOUR_VIN/command/stream`
